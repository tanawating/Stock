...
@section('content')
            ...
            <hr>
                <div class="panel-body">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                        @foreach($data_treasury as $key => $result)
                          <tr>
                            <td>{{ $result->name_treasury }}</td>
                            <td style="width: 15%">{{ $result->created_at }}</td>
                            <td style="width: 15%">{{ $result->updated_at }}</td>
                            <td style="width: 15%">
                            <a href="#" class="btn btn-success btn-sm" onclick="editData({{ $result->id}})"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></span> แก้ไข </a>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="form" class="form-horizontal" method="post" action="{!! URL::to('masterdata/add_treasury') !!}">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label class="col-sm-3 control-label">ชื่อคลัง</label>
                            <div class="col-sm-8">
...
<script type="text/javascript">
    ...
    {
        $.ajax({
            url: "{!! URL::to('/masterdata/treasury/detail') !!}/"+id, 
            success: function(response)
            {
                $('#id').val(response.data_treasury.id);
                $('#name').val(response.data_treasury.name_treasury);
                $('#state').val('update');
                $('#modalLabel').text('แก้ไขคลัง');
                $('#modal').modal('show');
            }
        });
    }
    ...
    function deleteData(id)
    {
        swal({
            ...
            {
                $.ajax({
                    url: "{!! URL::to('masterdata/treasury/destroy/') !!}/"+id,
                    success: function(response)
                    {
                        swal(
                                'สำเร็จ!',
...
{!! JsValidator::formRequest('App\Http\Requests\RequestMasterDataTreasury','#form') !!}
